# TITLE: 03_war_of_the_worlds >> solution_wotw.py
# AUTHOR: Chalmer Lowe
# DATE: 20181231
# DESCRIPTION:
...
fin = open('war_of_the_worlds.txt').readlines()

hashes = [(7, '9ae9bfa400f1d73faf0be494fa9f1b974f13b4ca9389492ff56d82832720a5e5'),
          (5, 'a1a10e9cc4d40413a79730dae2998da0558ed3d24faa8b52a2864fe0b38ed39d'),
          (2, 'b4d5f22189cfc169b8fa6b6eb1e0bbe38387083f04cd6fd8c69193567d03c0b5'),
          (3, 'f1fd23ddf471a9c260b6fe58a816a560b28f9cd7e657b5c5c87dc5e44a27f004'),
          (2, '48da7a4ee2e5b33a48f00e7333f882ce30c0db4dda84fd7457e65857ecc3a3d4'),
...
for line in fin:
    line = line.strip()
    hash_hexdigest = hashlib.sha256(line.encode()).hexdigest()
    words = line.split()
    for index, hexd in hashes:
        if hexd == hash_hexdigest:
...
keywords = []
for index, hash_obj in hashes:
    for line in fin:
        words = line.split()
        hash_object = hashlib.sha256(line.encode()).hexdigest()
        if hash_object == hash_obj:
            keywords.append(words[index])
            break
