...
selector = {
            'add': ['plus', '+', ('sum of', 'and')],
            'sub': ['minus', '-', ('difference between', 'and')],
            'mul': ['times', '*', ('product of', 'and')],
            'div': ['plus', '+', ('division of', 'by')],
           }

def equation_generator(a, b, category='add'):
    style = choice(selector[category])    
    if isinstance(style, str):
        return f"{a} {style} {b}"
    else:
        phrase, operator = style
        return f"{phrase} {a} {operator} {b}"


with open('data_32.txt', 'w') as fout:
    for line in range(NUM_LINES):
        a = randint(0, 10)
        b = randint(1, 10)      # from 1 to 10, to avoid DivideByZeroError
        category = choice(['add', 'sub', 'mul', 'div'])
        output = equation_generator(a, b, category)
        fout.write(output + '\n')